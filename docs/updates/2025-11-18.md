...
- **POS Frontend + Branches API**: POS UI sử dụng dữ liệu thật (products/customers/orders) và lưu invoice tabs theo org. Backend thêm module `BranchesController` (`GET /branches`) để nhân viên chọn chi nhánh trước khi tạo đơn POS (`channel=POS`, `isPaid=true`). Tests: `pnpm --filter @meocrm/web test`, `pnpm --filter @meocrm/api test`, Playwright POS spec đang cần env đăng nhập để chạy trọn (ghi chú trong PR).
- **Kiểm tra**:
